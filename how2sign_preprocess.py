...
def combine_frame(input_dir, seq_len):
    hand_pose_faces = []
    files_path = find_files(input_dir, pattern='**/*.json', interval=3)  # Adjust interval as needed
...
def prep(main_dir, output_dir, seq_len, num_workers=4):
    ...
    with Pool(num_workers) as pool:
        pool.starmap(extract_npy, [(output_dir, subfolder_path, seq_len) for subfolder_path in subfolders_path])
